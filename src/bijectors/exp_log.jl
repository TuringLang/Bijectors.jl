...
(b::Exp)(y) = exp.(y)
(b::Exp)(y::CuArray) = CUDAnative.exp.(y)
(b::Log)(x) = log.(x)
(b::Log)(x::CuArray) = CUDAnative.log.(x)
...
logabsdetjac(b::Log{0}, x::CuVector) = -CUDAnative.log.(x)
logabsdetjac(b::Log{1}, x::AbstractVector) = - sum(log.(x))
logabsdetjac(b::Log{1}, x::CuVector) = - sum(CUDAnative.log.(x))
logabsdetjac(b::Log{1}, x::AbstractMatrix) = - vec(sum(log.(x); dims = 1))
logabsdetjac(b::Log{1}, x::CuMatrix) = - vec(sum(CUDAnative.log.(x); dims = 1))
