...
@generated function transform(
    b::NamedTransform{names1}, x::NamedTuple{names2}
) where {names1,names2}
    exprs = []
    for n in names2
        if n in names1
...
@generated function with_logabsdet_jacobian(
    ...
    for n in names2
        if n in names1
            ...
            push!(
                body_exprs,
                :(($val_sym, $logjac_sym) = with_logabsdet_jacobian(b.bs.$n, x.$n)),
            )
...
@generated function with_logabsdet_jacobian(
    nc::NamedCoupling{target,deps,F}, x::NamedTuple
) where {target,deps,F}
    return quote
...
@generated function with_logabsdet_jacobian(
    ni::Inverse{<:NamedCoupling{target,deps,F}}, x::NamedTuple
) where {target,deps,F}
    return quote
