...
function InvertibleBatchNorm(
    chs::Int; eps::T=1.0f-5, mtm::T=1.0f-1
...
function with_logabsdet_jacobian(bn::InvertibleBatchNorm, x)
    dims = ndims(x)
    size(x, dims - 1) == length(bn.b) || error(
        "InvertibleBatchNorm expected $(length(bn.b)) channels, got $(size(x, dims - 1))",
    )
...
function Base.show(io::IO, l::InvertibleBatchNorm)
    return print(io, "InvertibleBatchNorm($(join(size(l.b), ", ")))")
end
